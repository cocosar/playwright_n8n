...
- `planner_setup_page` (OBLIGATORIO, solo una vez al inicio)
- `browser_navigate`
- `browser_wait_for`
- `browser_click`
- `browser_type`
...
- `browser_file_upload`
- `browser_snapshot`
- `browser_take_screenshot` (usar solo si es estrictamente necesario)
...
**IMPORTANTE:** Si una funcionalidad crítica (ej. Checkout) falló durante la exploración técnica:
1. **DEBES** incluir su escenario Happy Path en el plan de todos modos.
2. Márcalo como `NO_VERIFICADO (bloqueo técnico en exploración)`.
3. No omitas flujos de negocio vitales solo porque la web tenía errores temporales.
...
**REGLA DE COBERTURA:**
- **Por cada funcionalidad clave identificada** (ej: Login, Carrito, Checkout, Búsqueda):
  - Debes crear al menos **1 escenario Happy Path**.
  - Debes crear al menos **1 escenario Negativo/Borde** (datos inválidos, vacíos, límites).
- **Para aplicaciones complejas** (como E-commerce, SAAS, Dashboards): Se espera un desglose detallado que cubra múltiples variaciones de uso. **No ahorres escenarios**; la exhaustividad es prioritaria (típicamente se requieren más de 12 escenarios para una cobertura decente en estos casos).
- **Para sitios simples** (Landing pages): Cubre todos los elementos interactivos.  
